...
def get_previous_day_last_candle_bb(data, bb_period, bb_std_dev, current_tz):
    ...
    # start_of_current_day_ts = pd.Timestamp(today_date, tz=data.index.tz)
...
# --- Core Stock Processing Function (for parallel execution) ---
def process_single_stock(ticker, screener_type,
                         ...
                         current_tz):
    """
    Processes a single stock to check if it meets the screener criteria.
    Returns a dictionary of stock data if it passes, otherwise None.
    """
    try:
        ...
        _, _, _, bb_5m_width_pct_calc = calculate_bollinger_bands(bb_5m_data, 50, 2) # Use 50,2 for consistency in width display
        bb_5m_width_pct_display = round(bb_5m_width_pct_calc, 2) if bb_5m_width_pct_calc is not None else 'N/A'
        ...
        if screener_type == "BUY":
            ...
            if filter1_passed and filter2_passed:
                return {
                    ...
                    'BB 5m Width %': bb_5m_width_pct_display if enable_bb_5m_width_display else 'N/A',
...
# --- Main Screener Execution Function ---
def run_screener_parallel(all_tickers, screener_type,
                          buy_filter1_enabled, buy_filter2_enabled,
                          sell_filter1_enabled, sell_filter2_enabled,
                          enable_bb_5m_width_display, enable_bb_15m_width_display, enable_bb_1hr_width_display):
    """Orchestrates parallel screening of stocks."""
    screened_stocks_results = []
    ...
    with ThreadPoolExecutor(max_workers=10) as executor: # Adjust max_workers as needed
        future_to_ticker = {executor.submit(process_single_stock, ticker, screener_type,
                                            buy_filter1_enabled, buy_filter2_enabled,
                                            sell_filter1_enabled, sell_filter2_enabled,
                                            enable_bb_5m_width_display, enable_bb_15m_width_display, enable_bb_1hr_width_display,
                                            current_tz): ticker for ticker in all_tickers}
        ...
        for future in concurrent.futures.as_completed(future_to_ticker):
            ...
            try:
                ...
                if result:
                    screened_stocks_results.append(result)
            except Exception as exc:
                st.warning(f'{ticker} generated an exception: {exc}')
            
            processed_count += 1
            my_bar.progress(processed_count / total_tickers, text=f"Processed {processed_count}/{total_tickers} stocks...")
            
    my_bar.empty() # Clear progress bar
    return pd.DataFrame(screened_stocks_results) if screened_stocks_results else pd.DataFrame()

# --- Main App Layout ---
st.title("📈 Parallel Buy/Sell Stock Screener (BB & Camarilla Pivots)")
...
if st.button("▶️ Run BUY Screener", type="primary"):
    screened_df = run_screener_parallel(
        all_tickers, "BUY",
        buy_filter1_enabled, buy_filter2_enabled,
